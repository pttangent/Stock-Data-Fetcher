...
from .semantic import (
    extract_13f_relations,
    extract_8k_events,
    extract_offering_semantics,
    extract_proxy_semantics,
    extract_regulatory_semantics,
...
class DagWorkerMixin:
    def _discover_company(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        root, filings = archive.discover_filings(
            match["cik"],
            self.policy,
            refresh=bool(task["payload"].get("refresh")),
        ...
        for filing in filings:
            ...
            self.store.schedule_task(
                run_id=task["run_id"],
                task_type="download_filing",
                lane="download",
                priority=int(task["priority"]) + form_priority(form),
                symbol=symbol,
                cik=match["cik"],
                accession=accession,
                form=form,
    ...
    def _download_filing(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        filing_id = self.store.upsert_filing({
            "issuer_id": task["payload"]["issuer_id"],
            "security_id": task["payload"]["security_id"],
            "symbol": task["symbol"],
            "cik": task["cik"],
            ...
            "raw_storage_uri": evidence["storage_uri"],
            "raw_sha256": evidence["sha256"],
            "raw_bytes": evidence["bytes"],
    ...
    def _parse_filing(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        for index, doc in enumerate(documents, 1):
            ...
            document_rows.append({
                ...
                "is_attachment": int(not is_primary),
                "source_start_byte": doc["source_start_byte"],
                "source_end_byte": doc["source_end_byte"],
                "raw_sha256": doc["block_sha256"],
            ...
            if is_primary and doc["content_kind"] in {"html", "inline_xbrl", "text"}:
                ...
                for ordinal, section in enumerate(extract_sections(text, filing["form"], group), 1):
                    ...
                    section_rows.append({
                        ...
                        "text": section["text"],
                        "content_hash": sha256_bytes(section["text"].encode()),
                ...
                if doc["content_kind"] in {"html", "inline_xbrl"}:
                    for table_index, table in enumerate(extract_tables(payload), 1):
                        ...
                        table_rows.append({
                            ...
                            "rows_json": rows_json,
                            "rows_hash": sha256_bytes(rows_json.encode()),
                            "truncated": int(table["truncated"]),
                        })
                ...
                if root is not None:
                    for holding in parse_13f(root):
                        holding_rows.append({
                            ...
                            "filing_id": filing["filing_id"],
                            **{key: value for key, value in holding.items() if key != "row_index"},
                        })
        ...
        with self.store.connect() as conn:
            ...
            conn.execute(
                "UPDATE filing SET raw_sha256=?,raw_bytes=?,status='parsed' WHERE filing_id=?",
                (actual_sha, len(raw), filing["filing_id"]),
            )
            conn.commit()
    ...
    def _semantic_filing(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        if not filing:
            raise LookupError(task["accession"])
        if task["payload"].get("reset_semantics"):
            self.store.clear_semantic_filing(filing["filing_id"])
        ...
        for attempt in range(1, self.config.yahoo_max_attempts + 1):
            try:
                ...
                row = {
                    ...
                    "market_cap": _int(payload.get("marketCap")),
                    "enterprise_value": _int(payload.get("enterpriseValue")),
                    "sector_source": payload.get("sector"),
                    "industry_source": payload.get("industry"),
                    "business_summary": payload.get("longBusinessSummary"),
                    "raw_json": raw_json,
                }
    ...
    def _finalize_symbol(self, task: dict[str, Any]) -> dict[str, Any]:
        rows = self.store.query(
            """SELECT f.status,COUNT(*) n FROM filing f
               JOIN security s ON s.issuer_id=f.issuer_id
