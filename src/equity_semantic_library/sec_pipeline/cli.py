...
def build_parser() -> argparse.ArgumentParser:
    ...
    run = sub.add_parser("run", help="Read symbol metadata and run the online or local structure pipeline")
    ...
    run.add_argument(
        "--rebuild-semantic",
        action="store_true",
        help="In structure mode, also rebuild semantic output for filings already marked structured",
    )
    ...
    inspect = sub.add_parser("inspect", help="Inspect issuer-level output through a ticker/security view")
    inspect.add_argument("symbol")
    review_export = sub.add_parser("review-export", help="Export review_required semantic candidates as JSONL")
    review_export.add_argument("--output", required=True)
    review_export.add_argument("--limit", type=int)
    review_export.add_argument("--status", action="append", default=["review_required"])
    review_import = sub.add_parser("review-import", help="Import human/LLM candidate decisions from JSONL")
    review_import.add_argument("--input", required=True)
    review_import.add_argument("--reviewer-type", choices=["llm", "human"], required=True)
...
def validate_database(store: PipelineStore) -> dict:
    ...
    with store.connect() as conn:
        ...
        checks["orphan_candidates"] = conn.execute("SELECT COUNT(*) FROM semantic_candidate c LEFT JOIN evidence_snippet e ON e.evidence_id=c.evidence_id WHERE e.evidence_id IS NULL").fetchone()[0]
        checks["accepted_without_promotion"] = conn.execute(
            """SELECT COUNT(*) FROM semantic_candidate c
               LEFT JOIN candidate_promotion p ON p.candidate_id=c.candidate_id
               WHERE c.status IN ('rule_accepted','review_accepted') AND p.candidate_id IS NULL"""
        ).fetchone()[0]
        ...
        checks["sha_mismatch_issues"] = conn.execute(
            "SELECT COUNT(*) FROM pipeline_issue WHERE code='ValueError' AND message LIKE 'SHA256 mismatch%'"
        ).fetchone()[0]
        checks["candidate_status"] = {
            row[0]: row[1]
            for row in conn.execute("SELECT status,COUNT(*) FROM semantic_candidate GROUP BY status")
        }
        checks["candidate_levels"] = {
            row[0]: row[1]
            for row in conn.execute("SELECT promotion_level,COUNT(*) FROM semantic_candidate GROUP BY promotion_level")
        }
        checks["tables"] = {
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "run":
        if args.rebuild_semantic and _mode(args) != "structure":
            raise SystemExit("--rebuild-semantic is only valid with --mode structure")
    ...
    if args.command == "inspect":
        ...
        output = {
            "security": store.query(
                "SELECT s.*,i.* FROM security s JOIN issuer i ON i.issuer_id=s.issuer_id WHERE s.symbol=?",
                (symbol,),
            ),
            "filings": store.query(
                "SELECT accession,form,available_at,status FROM security_filing WHERE query_symbol=? ORDER BY available_at DESC",
                (symbol,),
            ),
            "assertions": store.query(
                """SELECT predicate,object_json,available_at,confidence,status
                   FROM security_semantic_assertion
                   WHERE query_symbol=? ORDER BY available_at DESC LIMIT 100""",
                (symbol,),
            ),
            "candidates": store.query(
                """SELECT c.predicate,c.object_json,c.available_at,c.confidence,c.status,c.promotion_level,c.section_role
                   FROM semantic_candidate c JOIN security s ON s.issuer_id=c.issuer_id
                   WHERE s.symbol=? ORDER BY c.available_at DESC LIMIT 100""",
                (symbol,),
            ),
            "relations": store.query(
                """SELECT target_entity_key,target_ticker,relation_type,available_at,confidence,status
                   FROM security_company_relation WHERE query_symbol=?
                   ORDER BY available_at DESC LIMIT 100""",
                (symbol,),
            ),
            "events": store.query(
                """SELECT event_type,item,event_time,title FROM security_event_ledger
                   WHERE query_symbol=? ORDER BY event_time DESC LIMIT 100""",
                (symbol,),
            ),
        }
